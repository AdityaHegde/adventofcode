...
func (s *sensor) isWithin(x, y int) bool {
  dist := utils2.ManhattanDist(x, y, s.x, s.y)
  return dist <= s.dist
}
...
func (p *problem) parseLine(line string) {
  ...
  sensorX := utils2.Int(matches[1])
  sensorY := utils2.Int(matches[2])
  p.add(sensorX, sensorY, sensorEntryType)
  beaconX := utils2.Int(matches[3])
  beaconY := utils2.Int(matches[4])
  p.add(beaconX, beaconY, beaconEntryType)

  dist := utils2.ManhattanDist(sensorX, sensorY, beaconX, beaconY)
...
func partTwo(p *problem) int {
  ...
  for i := 0; i < len(p.sensors); i++ {
    for j := i + 1; j < len(p.sensors); j++ {
      ...
      if utils2.ManhattanDist(s1.x, s1.y, s2.x, s2.y) != s1.dist+s2.dist+2 {
        continue
      }
      dx := int(math.Abs(float64(s2.x - s1.x)))
      xSign := utils2.Sign(s2.x - s1.x)
      dy := int(math.Abs(float64(s2.y - s1.y)))
      ySign := utils2.Sign(s2.y - s1.y)
      if dx < dy {
        for x := 0; x <= dx; x++ {
